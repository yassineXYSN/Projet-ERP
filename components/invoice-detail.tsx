...
export function InvoiceDetail({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    const fetchInvoice = async () => {
      ...
      if (fetchError) {
        console.log("[v0] Error fetching invoice:", fetchError.message)
      ...
      <div className="space-y-6 max-w-4xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">{error || "Cette facture n'existe pas ou a été supprimée."}</p>
          </div>
        </div>
        <Button asChild>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">{invoice.total_amount.toFixed(2)} DT</p>
            </div>
            <div>
              <p className="text-sm font-medium text-muted-foreground">Paid Amount</p>
              <p className="text-2xl font-bold mt-1 text-green-600">{invoice.paid_amount.toFixed(2)} DT</p>
            </div>
            <div>
